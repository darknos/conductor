...
2. Confirm the full test/validation suite is green locally before any push.
...
- Do not enable auto-merge; this repo may have no required checks so auto-merge
  can skip tests.
...
- Use review comment endpoints (not issue comments) to find inline feedback:
  ...
  - Reply to a specific review comment:
    ...
    gh api -X POST /repos/{owner}/{repo}/pulls/<pr_number>/comments \
      -f body='[conductor] <response>' -F in_reply_to=<comment_id>
    ```
- `in_reply_to` must be the numeric review comment id, not the GraphQL node id.
- All GitHub comments generated by the agent must be prefixed with `[conductor]`.
- If feedback requires changes:
  - Reply with intended fixes (`[conductor] ...`) **as an inline reply to the
    original review comment**.
  - Implement fixes, commit, push.
  - Reply with the fix details and commit sha (`[conductor] ...`).
...
- If review feedback expands scope, decide whether to include it now or defer
  ...
  call it out in the root-level `[conductor]` update with a brief reason.
