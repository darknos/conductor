...
name: linear
description: |
  Use the `linear_graphql` tool for raw Linear GraphQL operations such as
  comment editing, state transitions, and upload flows.
---

# Linear GraphQL

Use this skill for raw Linear GraphQL work during Conductor agent sessions.

## Primary tool

Use the `linear_graphql` client tool exposed by Conductor's agent session.
It reuses Conductor's configured Linear auth for the session.
