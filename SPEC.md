...
6. `Agent Runner`
   ...
   - Invokes the Claude Agent SDK `query()` call with the rendered prompt and agent config.
   - Iterates over SDK message stream and forwards events to the orchestrator.
...
4. `Execution Layer` (workspace + agent SDK)
   - Filesystem lifecycle, workspace preparation, Claude Agent SDK integration.
...
- Claude Agent SDK (`claude_agent_sdk` Python package or `@anthropic-ai/claude-agent-sdk` TypeScript
  package).
- `ANTHROPIC_API_KEY` environment variable for Claude Agent SDK authentication.
- Host environment authentication for the issue tracker.
...
- concurrency limits
- Claude Agent SDK options and timeouts
- workspace hooks
...
State tracked while a Claude Agent SDK session is running.

Fields:

- `session_id` (string, implementation-defined — e.g. UUID or SDK-provided identifier)
...
- `Session ID`
  - Implementation-defined (e.g. UUID or SDK-provided identifier). No fixed composition rule.
...
- `max_turns` (integer)
  - Default: `20`
  - Maps to Claude Agent SDK `max_turns` option.
...
- `permission_mode` (string)
  - Controls what the agent can do without operator confirmation.
  - Values: `"acceptEdits"`, `"bypassPermissions"`, `"plan"`, etc.
  - Default: implementation-defined.
- `allowed_tools` (list of strings)
  - Default: `["Read", "Edit", "Glob", "Grep", "Bash"]`
...
- `model` (string, optional)
  ...
  - Default: implementation-defined (e.g. `claude-sonnet-4-20250514`).
- `system_prompt` (string, optional)
  - Additional system prompt prepended to the workflow prompt.
...
- `max_budget_usd` (float, optional)
  - Maps to Claude Agent SDK `max_budget_usd` option.
- `env` (map string→string, optional)
  - Extra environment variables passed to the SDK.
- `sandbox` (map, optional)
  - Maps to Claude Agent SDK `SandboxSettings`.
...
- Claude Agent SDK is available (e.g. `ANTHROPIC_API_KEY` environment variable is set).
- `agent.permission_mode` is valid if specified.
...
- `agent.permission_mode`: string, default implementation-defined
- `agent.allowed_tools`: list of strings, default `["Read", "Edit", "Glob", "Grep", "Bash"]`
- `agent.disallowed_tools`: list of strings, optional
- `agent.model`: string, optional, default implementation-defined
- `agent.system_prompt`: string, optional
- `agent.turn_timeout_ms`: integer, default `3600000`
- `agent.stall_timeout_ms`: integer, default `300000`
- `agent.max_budget_usd`: float, optional
- `agent.env`: map string→string, optional
- `agent.sandbox`: map, optional
...
- If the issue is still in an active state, the worker should start another agent session in the
  same workspace, up to `agent.max_turns`.
...
- Before invoking the Claude Agent SDK, validate:
  - `cwd == workspace_path`
...
## 10. Agent Runner Protocol (Claude Agent SDK Integration)

This section defines the language-neutral contract for integrating the Claude Agent SDK.

Integration model:

- No subprocess launch. The runtime imports the Claude Agent SDK as a library.
- A `query()` call with `ClaudeAgentOptions` returns an async message iterator.
- The normative contract is message type handling, required behaviors, and the logical fields that
  must be extracted (session IDs, completion state, tool policy, and usage telemetry).

### 10.1 Integration Model

SDK integration parameters:

- Import the Claude Agent SDK library.
- Construct `ClaudeAgentOptions` from merged `agent` config + workspace path + rendered prompt.
- Call `query(prompt, options)` to get an async message iterator.
- No subprocess launch, no `bash -lc`, no stdio framing.

Notes:

- Authentication is handled by the SDK via the `ANTHROPIC_API_KEY` environment variable.
- Permission mode, cwd, and prompt are expressed as SDK options.

### 10.2 Session Initialization

No multi-step handshake. A single `query()` call with `ClaudeAgentOptions` starts the session:

```text
options = ClaudeAgentOptions(
  system_prompt=<system_prompt from agent config>,
  ...
  mcp_servers=<implementation-defined>,
)
...
- `session_id` is generated by the implementation (e.g. UUID) or derived from SDK metadata.
- Reuse the same session context for continuation turns within one worker run.

### 10.3 Message Processing

The client iterates over SDK message types instead of parsing line-delimited JSON.

SDK message types:

- `AssistantMessage` — agent reasoning and tool calls. Contains `.content` blocks which may include
  text and tool_use entries.
- `ResultMessage` — agent completed. `.subtype` indicates success or error.
- `StreamEvent` — granular streaming events (content_block_start, content_block_delta,
  content_block_stop).

Completion conditions:

- `ResultMessage` with `subtype="success"` -> success
- `ResultMessage` with `subtype="error"` -> failure
- Timeout (`turn_timeout_ms`) -> failure
- Iterator exhaustion -> success (normal completion)

Continuation processing:

- If the worker decides to continue after a successful completion, it should issue another
  `query()` call with continuation guidance (not the original task prompt).
- The SDK session does not persist across `query()` calls unless the implementation uses a
  `resume` or `continue_conversation` option.

### 10.4 Emitted Runtime Events (Upstream to Orchestrator)

The SDK client emits structured events to the orchestrator callback. Each event should include:
...
Events are sourced from SDK message types:

- `session_started` — emitted when `query()` begins
- `turn_completed` — on `ResultMessage` with `subtype="success"`
- `turn_failed` — on `ResultMessage` with `subtype="error"`
- `notification` — on `AssistantMessage` with text content
- Token usage extracted from `ResultMessage` or `AssistantMessage` metadata

### 10.5 Permission and Tool Policy

Permission, sandbox, and tool behavior is controlled by Claude Agent SDK options.

Policy configuration:

- `permission_mode` controls what the agent can do without operator confirmation.
- `allowed_tools` / `disallowed_tools` control tool availability.
- No manual approval request/response protocol — the SDK handles permissions internally based on
  the configured `permission_mode`.
- Custom tools can be provided via `mcp_servers` configuration.

Each implementation should document its chosen `permission_mode` and tool policy.

Optional MCP tool extension:

- An implementation may expose additional tools to the agent session via MCP servers.
- Current optional standardized tool: `linear_graphql` (exposed as an MCP server tool).
- If implemented, supported MCP servers should be passed to the SDK via `ClaudeAgentOptions`.
...
- `agent.turn_timeout_ms`: total turn/query timeout
...
The `Agent Runner` wraps workspace + prompt + Claude Agent SDK client.

Behavior:

1. Create/reuse workspace for issue.
2. Build prompt from workflow template.
3. Construct `ClaudeAgentOptions` from merged `agent` config and workspace path.
4. Call `query()` and iterate over the SDK message stream.
5. Forward events to orchestrator.
...
- Token usage data is extracted from Claude Agent SDK message metadata (typically from
  `ResultMessage` or `AssistantMessage` usage fields).
- Prefer absolute session totals when available.
- Extract input/output/total token counts from SDK message usage metadata.
- For absolute totals, track deltas relative to last reported totals to avoid double-counting.
- Do not treat generic `usage` maps as cumulative totals unless the message type defines them that
  way.
...
1. `Workflow/Config Failures`
   ...
   - Missing `ANTHROPIC_API_KEY` or SDK not available
...
- Implementations should state clearly whether they rely on a permissive `permission_mode`,
  restricted tool lists, stricter sandboxing, or some combination of those controls.
...
- Agent SDK `cwd` must be the per-issue workspace path for the current run.
...
- Restricting Claude Agent SDK `permission_mode` and `sandbox` settings described elsewhere in this
  specification instead of running with a maximally permissive configuration.
- Adding external isolation layers such as OS/container/VM sandboxing, network restrictions, or
  separate credentials beyond the built-in SDK policy controls.
...
function run_agent_attempt(issue, attempt, orchestrator_channel):
  ...
  options = build_claude_options(workspace.path, config.agent)
  ...
  while true:
    ...
    for message in query(prompt, options):
      send(orchestrator_channel, {agent_update, issue.id, message})
...
- `agent.permission_mode` is validated when specified
...
### 17.5 Claude Agent SDK Client

- SDK initialization succeeds when `ANTHROPIC_API_KEY` is set and SDK is available
- SDK initialization fails gracefully with `sdk_initialization_error` or `api_key_missing`
- `query()` invocation uses workspace path as `cwd`
- `ClaudeAgentOptions` correctly maps `agent` config fields (permission_mode, allowed_tools, model,
  max_turns, sandbox, etc.)
- `AssistantMessage`, `ResultMessage`, and `StreamEvent` types are handled correctly
- `ResultMessage` with `subtype="success"` produces `turn_completed` event
- `ResultMessage` with `subtype="error"` produces `turn_failed` event
- Turn timeout is enforced
- `permission_mode` validation rejects invalid values
- Usage data is extracted from SDK message metadata
- If optional MCP server tools are implemented, they are passed via `ClaudeAgentOptions`
- If the optional `linear_graphql` MCP tool extension is implemented:
  - the tool is available to the agent session
...
- Claude Agent SDK client with `query()` async iterator
- Claude Agent SDK config (`agent.permission_mode`, `agent.allowed_tools`, `agent.model`, etc.)
